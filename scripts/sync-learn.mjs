 ...
 *   $GITHUB_WORKSPACE/learn/        ← nodejs/learn checkout (cwd when running)
 ...
 *
 * Only recipes whose README ends with `<!-- sync_to_learn: true -->` are
 * published; recipes without the marker are treated as opt-out.
 *
 * Run from the nodejs/learn checkout root:
 ...
 * Inline code markers are stripped, since sidebar labels
 * in site.json are rendered as plain text.
 */
...
/**
 * Reads the `<!-- sync_to_learn: boolean -->` marker that every README
 * carries. A missing or malformed marker means "do not publish", so a new
 * recipe has to opt in explicitly.
 */
function shouldSync(content) {
  const match = content.match(/<!--\s*sync_to_learn:\s*(true|false)\s*-->/);
  return match?.[1] === "true";
}
...
for (const name of recipes) {
  ...
  if (!shouldSync(content)) {
    console.log(`  – ${name} (sync_to_learn is not true)`);
    continue;
  }
