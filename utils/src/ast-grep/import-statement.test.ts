...
describe("import-statement", () => {
	...
	it("should get variable declarator with module", () => {
		...
		const moduleCalls = getNodeImportCalls(ast, "fs");
		assert.strictEqual(moduleCalls.length, 1, "dynamic import using variable need to be caught");
	});
	...
	it("should not capture function calls that are not import() expressions", () => {
		...
		const moduleCalls = getNodeImportCalls(ast, "fs");
		assert.strictEqual(moduleCalls.length, 0, "functions that aren't import shouldn't be caught");
	});
