...
export default function transform(root: SgRoot<JS>): string | null {
	...
	const hasESMImports =
		getNodeImportStatements(root, 'test').length > 0 ||
		rootNode.findAll({ rule: { kind: 'import_statement' } }).length > 0;
	const hasESMExports =
		rootNode.findAll({ rule: { kind: 'export_statement' } }).length > 0;
	...
	// Transform done callbacks and this.skip() with AST
	transformDoneCallbacks(rootNode, edits);
	transformThisSkip(rootNode, edits);

	// Commit AST edits
	let sourceCode =
		edits.length > 0 ? rootNode.commitEdits(edits) : rootNode.text();

	// Transform timeouts using string manipulation (both adding options and removing statements)
		...
		if (functionNode) {
			...
			if (params) {
				...
				if (paramsText === '()' || paramsText === '( )') {
					...
					!paramsText.includes('t,') &&
					!paramsText.startsWith('(t)')
				) {
 ...
 * AND removes the this.timeout() statements
 */
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (i + 1 < lines.length) {
			...
			if (timeoutMatch) {
				...
				// Try to match regular function pattern with description
				...
				if (testMatch) {
					...
					result.push(
						`${indent}${funcName}${modifier}(${description}, { timeout: ${timeout} }, ${async}function${params} {`,
					);
				...
				// Try to match arrow function pattern with description
				...
				if (testMatch) {
					...
					result.push(
						`${indent}${funcName}${modifier}(${description}, { timeout: ${timeout} }, ${async}${params} => {`,
					);
					i++; // Skip the timeout line
					if (i + 1 < lines.length && lines[i + 1].trim() === '') {
						i++;
				...
				if (testMatch) {
					...
					result.push(
						`${indent}${funcName}${modifier}({ timeout: ${timeout} }, ${async}function${params} {`,
					);
					i++; // Skip the timeout line
					if (i + 1 < lines.length && lines[i + 1].trim() === '') {
						i++;
				...
				if (testMatch) {
					...
					result.push(
						`${indent}${funcName}${modifier}({ timeout: ${timeout} }, ${async}${params} => {`,
					);
					i++; // Skip the timeout line
					if (i + 1 < lines.length && lines[i + 1].trim() === '') {
						i++;
