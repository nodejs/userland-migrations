...
const callMetric = useMetricAtom('fs-truncate-fd-calls');
const filesMetric = useMetricAtom('fs-truncate-fd-files');
 ...
 */
export function isLikelyFileDescriptor(param: string, rootNode: SgNode<Js>): boolean {
 ...
 */
export function isInCallbackContext(param: string, rootNode: SgNode<Js>): boolean {
 ...
 * @param rootNode The root node of the AST
 */
export function isAssignedFromOpenSync(param: string, rootNode: SgNode<Js>): boolean {
		...
		for (const check of checks) {
			...
			for (const call of calls) {
				...
				if (check.isSync) {
					...
					callMetric.increment({ fn: 'truncateSync' });
				} else {
					usedTruncate = true;
					callMetric.increment({ fn: 'truncate' });
				}
			}
