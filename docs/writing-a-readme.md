...
Every recipe directory must have a `README.md` that explains what the migration does, how to run it, and what the transformation looks like. This document describes the required structure, section-by-section writing guidance, and the conventions used across the repository.

Recipe READMEs are also published as articles on [nodejs.org/learn](https://nodejs.org/learn), so each one must be a self-contained page that reads well outside the repository. See [Publishing to nodejs.org/learn](#publishing-to-nodejsorglearn) for the constraints that come with that.
...
```sh
npx codemod @nodejs/<codemod-name>
```
...
When a recipe has multiple authors, list them on the same line, separated by commas:

```yaml
---
authors: github-username-one, github-username-two
---
```

Do not use a YAML sequence (`- github-username`): nodejs.org/learn parses `authors` as a single comma-separated string, and a list breaks the page's author attribution.
...
**1. `` DEPXXXX: `old-api` to `new-api` `` - for single-deprecation replacements**
...
```markdown
# DEP0147: `fs.rmdir()` to `fs.rm()`

# DEP0178: `dirent.path` to `dirent.parentPath`

# DEP0093: `crypto.fips` to `crypto.getFips()` / `crypto.setFips()`
```
...
# DEP0191: `repl.builtinModules` / `repl._builtinLibs` to `module.builtinModules`
```
...
- Join the old and new API with the word `to`.
...
The codemod name is the `name` field declared in the recipe's `codemod.yaml` — not the directory name. The two usually match, but the registry name is what `npx codemod` resolves, and it is case-sensitive (for example, the recipe in `recipes/tmpdir-to-tmpdir/` is published as `@nodejs/tmpDir-to-tmpdir`). Always copy the name from `codemod.yaml`. For example, the recipe in `recipes/rmdir/` declares `name: "@nodejs/rmdir"` and is run with:
...
The `Examples` section is the most important part of the README. It shows exactly what the transformation does by deriving examples directly from the recipe's test fixtures.

### How to derive examples from test fixtures

Each recipe organises its tests as one directory per case, named for what the case covers, with an `input` and an `expected` file inside: `tests/<test-case-description>/input.ts` and `tests/<test-case-description>/expected.ts` (the extension matches the fixture's language). Read those files and construct `diff` blocks by diffing them line by line. Lines that are removed carry a `-` prefix; lines that are added carry a `+` prefix; lines that are unchanged and provide useful context carry a leading space.

For example, `tests/file-1/input.js` for the `rmdir` recipe contains:
...
The corresponding `tests/file-1/expected.js` contains:
...
## Publishing to nodejs.org/learn

Recipe READMEs are copied verbatim into [nodejs/learn](https://github.com/nodejs/learn) (built with [doc-kit](https://github.com/nodejs/doc-kit)) on every publish, where each one renders as an article. That imposes a few hard rules:

- Exactly one `#` (H1) heading per file — it becomes the page title and the sidebar label.
- `authors` in the frontmatter must be a single-line, comma-separated string of GitHub usernames (see [Frontmatter](#frontmatter)); the site uses it to render linked avatars.
- Every code fence must declare a language (`js`, `sh`, `diff`, `yaml`, ...).
- GitHub-style alerts (`> [!NOTE]`, `> [!TIP]`, `> [!IMPORTANT]`, `> [!CAUTION]`) are supported and render as call-out boxes.
- Use `-` for bullet lists, and do not prefix shell commands with `$`.
- The page must make sense on its own: spell out repository-relative context (a learn reader cannot see the recipe's source tree).
...
- [ ] Opens with a `---`-delimited frontmatter block containing `authors: <github-username>` (comma-separated on one line when there are several).
- [ ] Has a title that follows the naming convention for its type (DEP number + `to`, DEP number + description, or plain descriptive) with API names in backticks.
- [ ] Has a description paragraph that names the deprecated API, its replacement, and any notable variant handling.
- [ ] Has a `## Usage` section whose `npx codemod` command matches the `name` field in the recipe's `codemod.yaml`.
